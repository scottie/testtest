...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Zix address (e.g. %1)").arg("PCYiHgGJJ6xGHqivmdZrYjRnhaYf6AJ2Mp"));
#endif
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient* out)
{
    // Convert zix:// to zix:
    //
    //    Cannot handle this later, because zix:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open zix.conf with the associated application */
...
{
    return GetAutostartDir() / "zix.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a zix.desktop file to the autostart directory:
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the zix app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add zix app to startup item list
