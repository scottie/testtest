...
#include "ui_zzixcontroldialog.h"
...
std::list<std::string> ZZixControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZZixControlDialog::listMints;

ZZixControlDialog::ZZixControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZZixControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZZixControlDialog::~ZZixControlDialog()
{
    delete ui;
}

void ZZixControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZZixControlDialog::updateList()
{
...
void ZZixControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZZixControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZZix_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZZixControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZZixControlDialog::GetSelectedMints()
{
...
void ZZixControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
