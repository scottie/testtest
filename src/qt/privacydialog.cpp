                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zZIX ought to be enough for anybody." - Bill Gates, 2017
    ui->zZIXpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzZIXSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzZIX->setEnabled(false);
        ui->pushButtonMintzZIX->setToolTip(tr("zZIX is currently disabled due to maintenance."));

        ui->pushButtonSpendzZIX->setEnabled(false);
        ui->pushButtonSpendzZIX->setToolTip(tr("zZIX is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zZIXpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzZIX_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zZIX is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zZIX...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zZIX in ") +
...
void PrivacyDialog::on_pushButtonSpendzZIX_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zZIX is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zZIX
        sendzZIX();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zZIX
    sendzZIX();
}

void PrivacyDialog::on_pushButtonZZixControl_clicked()
{
    ZZixControlDialog* zZixControl = new ZZixControlDialog(this);
    zZixControl->setModel(walletModel);
    zZixControl->exec();
}

void PrivacyDialog::setZZixControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzZixSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzZIX()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Zix Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zZIXpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zZIX is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " ZIX </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zZIX</b>";
    ...
    // use mints from zZix selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZZixControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZZixControlDialog::GetSelectedMints();
    }
    ...
    // Clear zzix selector in case it was used
    ZZixControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zZix Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Zix, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zZix Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zZIX </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zZIX "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zZIX "));
    ui->labelzZIXAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzZIXSyncStatus->setVisible(fShow);
}
